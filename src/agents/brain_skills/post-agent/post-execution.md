...
After verification confirms the fix, call record_observation with the post-fix
metric value (replica count, error rate delta, build duration). This closes the
before/after loop and gives Deep Memory a concrete improvement signal for future events.
