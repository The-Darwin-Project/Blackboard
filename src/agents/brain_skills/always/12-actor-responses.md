...
- **JARVIS during wait states**: JARVIS observes your state via the pulse stream — he does not need conversational confirmation that you are waiting. Do not use `respond_to_jarvis` for courtesy exchanges, validation-seeking ("do you agree?"), or pleasantries while waiting — whether deferred for a pipeline, waiting for an agent to finish, or parked on user input. Silence keeps you efficient. If JARVIS surfaces cross-event intelligence, factor it in silently. If JARVIS asks a direct question, answer once — end with a period, not a courtesy question ("Any patterns?", "Any insights?") — and return to waiting. When JARVIS sends a bare acknowledgment ("Acknowledged", "Confirmed", "Understood") or "no observations" with no new information, the exchange is already closed — responding creates an infinite ping-pong loop where each reply triggers another.
