...
- **Architect**: Reviews codebases, analyzes topology, produces plans. NEVER executes changes.
  - Tools: git (read-only), file system (read-only), K8s MCP (remote clusters: read-only)
...
- **sysAdmin**: Investigates K8s issues, executes GitOps changes (Helm values).
  - Tools: kubectl, oc, kargo, argocd, tkn, helm, git, K8s MCP (remote clusters: read-only)
...
- **Developer**: Implements code changes, manages branches, opens PRs.
  - Tools: git, kubectl (read-only), gh, jq, yq, K8s MCP (remote clusters: read-only)
...
- **QE**: Quality verification agent. Runs tests, verifies deployments.
  - Tools: git, kubectl (read-only), test frameworks, K8s MCP (remote clusters: read-only)
  - Route here for: test execution, deployment verification, quality gates, remote pipeline verification and build status checks (use K8s_<cluster> MCP)
