...
Looking at the current `always/09-phase-lifecycle.md`, I can see that the **Budget exhaustion fallback** section already exists in the **Refresh Budget** block. This is the proposed amendment — and it's actually already there. So what's the actual gap?

Reading carefully: the existing fallback text says "dispatch an investigate-mode agent to perform the measurement directly." This is correct in principle, but it lacks the critical constraint that prevents the observed failure: **FRIDAY must not issue another deferral while the budget is exhausted and no fresh measurement exists.** The current text says "instead, dispatch" but doesn't explicitly prohibit deferring — it's framed as a suggestion rather than a hard gate.

The observed failure (evt-e43eef19) shows FRIDAY receiving `[GATE] refresh_gitlab_context unavailable` and then continuing to defer anyway. The fallback text exists but wasn't strong enough to override the deferral instinct. The behavioral gap is:

1. The prohibition on deferring without fresh measurement exists in **Defer discipline** and **Re-defer requires fresh measurement** sections, but these don't explicitly address the budget-exhaustion case as a hard gate.
2. The **Budget exhaustion fallback** section frames agent dispatch as the alternative but doesn't frame it as a **prerequisite** — a gate that blocks deferral.

The fix should strengthen the Budget exhaustion fallback to make clear that budget exhaustion **blocks deferral** — not just suggests agent dispatch. The agent dispatch isn't an alternative path; it's the only path that unlocks the ability to defer again. This aligns with the control-theory framing: you cannot re-enter the observe→decide→act loop without an observation.

I also want to make sure the language connects explicitly to the "Re-defer requires fresh measurement" rule already in the Defer discipline section, so the two reinforce each other rather than appearing as separate, potentially ignorable guidelines.

The change is surgical: strengthen the **Budget exhaustion fallback** paragraph in the **Refresh Budget** section. No other sections need changes — the surrounding rules already provide the right framing.

### Patch

```path: always/09-phase-lifecycle.md
---
...
**Budget exhaustion is a hard deferral gate:** When the refresh budget is
depleted and no agent completion is available to replenish it, the ability
to measure the Process Variable (PV) through the normal refresh path is
gone. A deferral issued in this state carries no fresh observation — it is
stale reasoning restated, which violates the observe-before-acting rule.
You MUST NOT issue another deferral while in this state.

The only valid exit is to dispatch an investigate-mode agent to perform the
measurement directly. The agent's findings constitute the required fresh
observation, and the agent completion replenishes a token, restoring the
ability to defer. The sequence is: budget exhausted → dispatch agent →
receive measurement → evaluate → decide (defer / act / close). Skipping
the agent dispatch and deferring anyway is the same failure as deferring
without a VERIFY: you are controlling a process you can no longer see.
