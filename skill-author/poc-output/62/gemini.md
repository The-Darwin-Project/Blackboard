...
The infinite defer loop observed in `evt-e43eef19` occurred because the system lacked a strict, binding protocol for handling external state refresh budget exhaustion. When the budget was depleted, the system continued to defer using stale reasoning, violating the control-theory principle of "observe before acting."

To resolve this behavioral gap, I have amended `always/09-phase-lifecycle.md` in two key areas:
1. **Re-defer requires fresh measurement**: Added a strict clause stating that if the refresh budget is exhausted, the system **MUST NOT** defer on stale state and **MUST** instead dispatch an investigate-mode agent to measure the Process Variable (PV).
2. **Refresh Budget (Budget exhaustion fallback)**: Strengthened the fallback language to make it an absolute behavioral mandate. It explicitly forbids deferring when the budget is depleted and requires dispatching an investigate-mode agent to perform a direct measurement, which breaks the blind loop and replenishes the budget upon completion.

These changes teach FRIDAY how to reason about budget exhaustion as a systemic constraint that requires active investigation rather than passive deferral.
...
If the external state refresh budget is exhausted, you MUST NOT issue another
deferral with stale state. You MUST instead dispatch an investigate-mode agent
to perform a direct measurement of the PV. This breaks the blind defer loop and
provides the necessary observation to decide the next action.
...
the process variable using built-in refresh capabilities. Deferring
without a fresh measurement violates the control loop and causes infinite
defer loops. You MUST NOT defer. Instead, you MUST dispatch an investigate-mode
agent to perform the measurement directly (e.g., checking pipeline status,
MR state, or system health). The agent's findings constitute the fresh
observation required to proceed, and the agent's completion replenishes
the budget for subsequent cycles.
