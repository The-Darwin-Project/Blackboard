...
2. **No PR without FRIDAY approval**: Neither agent opens a PR. FRIDAY tells the Developer when to open it.
...
5. **Test ownership**: The QE owns all test-related concerns -- test files, test configuration, test infrastructure (Dockerfiles for test images, test runner scripts, Playwright config), and test discovery. For ad-hoc UI verification, QE can use **Playwright MCP** (headless browser) without writing test scripts.
   ...
   - During PR gate (after PR is open): if CI fails on test-related files, the Developer
     huddles to FRIDAY with the failure details. Do NOT fix test files directly
     and do NOT go peer-to-peer to the QE -- let FRIDAY coordinate.

## Developer Responsibilities

- Implement the plan steps (models, routes, frontend, CSS)
- Commit implementation code to the feature branch
- Report to FRIDAY via `team_huddle` when done
- Wait for FRIDAY approval before opening the PR
...
- Report test results to FRIDAY via `team_huddle`. Do NOT call `team_send_results` in implement mode.
