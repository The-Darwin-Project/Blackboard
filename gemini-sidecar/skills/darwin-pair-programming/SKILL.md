...
Both agents share the same filesystem(Volume). Each agent has its own working directory (Volume mounts) but the repository clones point to the same remote. Both push to the same `feat/evt-{EVENT_ID}` branch on origin.

## Coordination Rules

1. **Same branch**: Both agents MUST commit to `feat/evt-{EVENT_ID}`. Read the event ID from `events/event-{id}.md`.
...
4. **Git pull before push**: Always `git pull --rebase origin feat/evt-{EVENT_ID}` before pushing to avoid conflicts with your partner's commits.
