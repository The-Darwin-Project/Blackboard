        ...
        <table className="w-full text-xs" style={{ tableLayout: 'auto' }}>
          ...
          <tbody>
            {memories.map((m) => {
              ...
              return (
                <Fragment key={m.id}>
                  <tr className="border-b border-border hover:bg-bg-tertiary cursor-pointer transition-colors align-top"
                    onClick={() => toggle(eid)}>
                    ...
                    <td className="px-3 py-2 whitespace-nowrap text-text-secondary">{String(p.service || '?')}</td>
                    <td className="px-3 py-2 text-text-secondary break-words max-w-[200px]">
                      {String(p.symptom || '?').slice(0, 80)}
                    </td>
                    <td className="px-3 py-2 text-text-secondary break-words max-w-[200px]">
                      {String(p.root_cause || '?').slice(0, 80)}
                    </td>
                    ...
                    <tr className="border-b border-border bg-bg-secondary">
                      <td colSpan={6} className="px-4 py-3">
                        <div className="grid grid-cols-2 gap-3 text-xs mb-3">
                          <div><span className="text-text-muted">Event:</span> <span className="text-text-primary font-mono">{String(p.event_id)}</span></div>
                          <div><span className="text-text-muted">Service:</span> <span className="text-text-primary">{String(p.service)}</span></div>
                          <div><span className="text-text-muted">Domain:</span> <span className="text-text-primary">{String(p.brain_domain || p.domain)}</span></div>
                          <div><span className="text-text-muted">Outcome:</span> <span className="text-text-primary">{String(p.outcome)}</span></div>
                          <div className="col-span-2"><span className="text-text-muted">Pattern:</span> <span className="text-text-primary">{String(p.symptom)}</span></div>
                          <div className="col-span-2"><span className="text-text-muted">Root cause:</span> <span className="text-text-primary">{String(p.root_cause)}</span></div>
                          <div className="col-span-2"><span className="text-text-muted">Fix:</span> <span className="text-text-primary">{String(p.fix_action)}</span></div>
                        ...
                        {!form ? (
                          <button onClick={(e) => { e.stopPropagation(); startCorrection(eid, String(p.root_cause || ''), String(p.fix_action || '')); }}
