...
export const MOCK_EVENTS = [
  {
    ...
    reason: 'Split backend into inventory and customer services',
    evidence: { display_text: 'User requested backend service split for inventory-api', domain: 'complex', severity: 'info' },
  ...
  },
  {
    ...
    source: 'headhunter',
    service: 'cnv-operator',
    current_agent: 'developer',
    reason: 'Pipeline failed on MR !470',
    evidence: { display_text: 'Pipeline failed: ETXTBSY on linux-mxlarge runners', domain: 'complicated', severity: 'warning' },
  ...
  },
  {
    ...
    evidence: { display_text: 'Memory usage 92% — waiting for autoscaler response', domain: 'clear', severity: 'warning' },
...
export const MOCK_CLOSED_EVENTS = [
  {
    ...
    current_agent: null,
    reason: 'Investigate intermittent 502s',
    evidence: { display_text: 'Resolved: load balancer health check timeout', domain: 'complicated', severity: 'info' },
  ...
  },
  {
    ...
    current_agent: null,
    reason: 'MR !301 pipeline passed',
    evidence: { display_text: 'Konflux Release bundle v4.19.1-rc2 merged successfully', domain: 'clear', severity: 'info' },
    created: new Date(Date.now() - 22 * 60_000).toISOString(),
  },
];
